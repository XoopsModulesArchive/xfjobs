...
$GLOBALS['xoopsOption']['template_main'] = 'xfjobs_index.html';
...
function deleteJob($jobId)
{
    ...
    $result = $xoopsDB->queryF(
        'DELETE FROM ' . $xoopsDB->prefix('xf_people_job') . " WHERE job_id='$jobId'"
    );
    ...
    if (!$result) {
        $xoopsForgeErrorHandler->addError(
            "Failed deleting job $jobId: " . 'Does not exist'
        );
    ...
    $xoopsDB->queryF(
        'DELETE FROM ' . $xoopsDB->prefix('xf_people_job_inventory') . " WHERE job_id='$jobId'"
    );
