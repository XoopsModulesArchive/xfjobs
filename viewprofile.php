...
$GLOBALS['xoopsOption']['template_main'] = 'xfjobs_viewprofile.html';
...
if ($user_id) {
    ...
    */
    ...
    $sql = 'SELECT user_id,uname,people_view_skills,resume FROM ' . $xoopsDB->prefix('users') . ',' . $xoopsDB->prefix('xf_user_profile') . ' ' . "WHERE uid='$user_id' " . 'AND uid=user_id';
    ...
    if (!$result || $xoopsDB->getRowsNum($result) < 1) {
        ...
        */
        ...
        if (1 != unofficial_getDBResult($result, 0, 'people_view_skills')) {
		...
		<TR><TD>
			...
			' . $ts->displayTarea(unofficial_getDBResult($result, 0, 'resume')) . '
		</TD></TR>
		<TR><TD>
