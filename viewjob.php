...
$GLOBALS['xoopsOption']['template_main'] = 'xfjobs_viewjob.html';
...
if ($group_id && $job_id) {
    ...
    if (!$result || $xoopsDB->getRowsNum($result) < 1) {
        ...
        if ($deljob) {
            $content .= '<h4><b><a href="' . XOOPS_URL . '/modules/xfjobs/?group_id=' . $group_id . '&del_job_id=' . $job_id . '">Click here to delete this job</a></b></h4>';
        } else {
            $content .= '<h4>' . _XF_PEO_VIEWAJOB . '</h4>';
        }
		...
		<p><table border="0" width="100%"><tr><td colspan="2"><b>'
                    . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'title'))
                    . '</b>
		</td></tr><tr><td width="50%"><b>'
                    . _XF_PEO_CONTACTINFO
                    . ':<br>
		<a href="javascript:openWithSelfMain(\''
                    . XOOPS_URL
                    . '/pmlite.php?send2=1&to_userid='
                    . unofficial_getDBResult($result, 0, 'uid')
                    . '\',\'pmlite\',450,450);">'
                    . unofficial_getDBResult($result, 0, 'uname')
                    . '</a></b></td><td width="50%"><b>'
                    . _XF_PEO_STATUS
                    . ':</b><br>'
        ...
        if ($xoopsUser) {
            ...
            if (!$apply) {
                ...
                $content .= '<tr><td colspan="2">'
                            ...
                            . $job_id
                            . '">'
                            . '<div align="center"><input type="submit" name="apply" value="'
                            . _XF_PEO_APPLY
                            . "\"></div></form></td></tr>\n";
            } else {
                $skill_sql = 'SELECT s.name as skill_name,'
                                ...
                                . ' sy, '
                                . $xoopsDB->prefix('xf_people_skill_inventory')
                                . ' si'
                                . " WHERE si.user_id='"
                                . $xoopsUser->uid()
                                . "'"
                ...
                $mail = $subject
                                ...
                                . _XF_PEO_JOB
                                . ':  '
                                . unofficial_getDBResult($result, 0, 'title')
                                . "\n"
                                . _XF_PEO_PROJECT
                                . ':  '
                                . unofficial_getDBResult($result, 0, 'group_name')
                ...
                if ($xoopsDB->getRowsNum($skill_result) > 0) {
                    $mail .= _XF_PEO_SKILLS . ":\n";

                    while (false !== ($skill_row = $xoopsDB->fetchArray($skill_result))) {
                        $mail .= "\t" . $skill_row['skill_name'] . ' - ' . $skill_row['skill_level_name'] . ' (' . $skill_row['skill_year_name'] . ")\n";
                    }
                }

                $mail .= "\n\n" . _XF_PEO_ADDVIAPRJPAGE . ' (' . XOOPS_URL . '/modules/xfmod/project/?' . unofficial_getDBResult($result, 0, 'unix_group_name') . ').' . "\n\n" . _XF_PEO_THANKSFORUSING . ' ' . $xoopsConfig['sitename'] . ".\n";

                $adm_sql = 'SELECT u.uid ' . ' FROM ' . $xoopsDB->prefix('users') . ' u, ' . $xoopsDB->prefix('xf_user_group') . ' ug' . " WHERE ug.group_id='" . $group_id . "'" . " AND ug.admin_flags='A'" . ' AND u.uid=ug.user_id';

                $adm_result = $xoopsDB->query($adm_sql);

                $xoopsMailer = getMailer();

                while (false !== ($adm_row = $xoopsDB->fetchArray($adm_result))) {
                ...
                $content .= '<tr><td colspan="2">' . '<b>' . _XF_PEO_APPSUBMITTED . '</b></td></tr>';
            }
        }
