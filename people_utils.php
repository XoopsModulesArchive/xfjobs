...
function people_add_to_skill_inventory($skill_id, $skill_level_id, $skill_year_id)
{
    ...
    if ($xoopsUser) {
        ...
        $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_people_skill_inventory') . ' ' . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . "AND skill_id='$skill_id'";
        ...
        if (!$result
            ...
            $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_people_skill_inventory') . ' (user_id,skill_id,skill_level_id,skill_year_id) ' . "VALUES ('" . $xoopsUser->getVar('uid') . "','$skill_id','$skill_level_id','$skill_year_id')";
...
function people_show_skill_inventory($user_id)
{
    ...
    $sql = 'SELECT ps.name AS skill_name, psl.name AS level_name, psy.name AS year_name ' . 'FROM ' . $xoopsDB->prefix('xf_people_skill_year') . ' psy,' . $xoopsDB->prefix('xf_people_skill_level') . ' psl,' . $xoopsDB->prefix('xf_people_skill') . ' ps,' . $xoopsDB->prefix(
        'xf_people_skill_inventory'
    ) . ' psi ' . 'WHERE psy.skill_year_id=psi.skill_year_id ' . 'AND psl.skill_level_id=psi.skill_level_id ' . 'AND ps.skill_id=psi.skill_id ' . "AND psi.user_id='$user_id'";

    $result = $xoopsDB->query($sql);

    $content = "<TABLE cellspacing='1' cellpadding='5' width='100%' border='0'>" . "<TR class='bg2'>" . "<TD align='center'><b>" . _XF_PEO_SKILL . '</b></td>' . "<td align='center'><b>" . _XF_PEO_LEVEL . '</b></td>' . "<TD align='center'><b>" . _XF_PEO_EXPERIENCE . '</b></td>' . '</TR>';
...
function people_edit_skill_inventory($user_id)
{
    ...
    if (!$result || $rows < 1) {
        $content .= '
			<TR><TD COLSPAN="4"><H4>' . _XF_PEO_NOSKILLINVENTORYSETUP . '</H4></TD></TR>';
        ...
        for ($i = 0; $i < $rows; $i++) {
			...
			<TR class="' . (0 != $i % 2 ? 'bg2' : 'bg3') . '">
				<TD><FONT SIZE="-1">' . people_get_skill_name(
				...
				<TD><FONT SIZE="-1">' . people_skill_level_box(
                    'skill_level_id',
                    unofficial_getDBResult($result, $i, 'skill_level_id')
                ) . '</TD>
				<TD><FONT SIZE="-1">' . people_skill_year_box(
                            'skill_year_id',
                            unofficial_getDBResult($result, $i, 'skill_year_id')
                        ) . '</TD>
...
function people_add_to_job_inventory($job_id, $skill_id, $skill_level_id, $skill_year_id)
{
    ...
    if ($xoopsUser) {
        ...
        if (!$result || $xoopsDB->getRowsNum($result) < 1) {
            ...
            $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_people_job_inventory') . ' (job_id,skill_id,skill_level_id,skill_year_id) ' . "VALUES ('$job_id','$skill_id','$skill_level_id','$skill_year_id')";
...
function people_show_job_inventory($job_id)
{
    ...
    $sql = 'SELECT ps.name AS skill_name, psl.name AS level_name, psy.name AS year_name '
           . 'FROM '
           . $xoopsDB->prefix('xf_people_skill_year')
           . ' psy,'
           . $xoopsDB->prefix('xf_people_skill_level')
           . ' psl,'
           . $xoopsDB->prefix('xf_people_skill')
           . ' ps,'
    ...
    if (!$result || $rows < 1) {
        ...
        for ($i = 0; $i < $rows; $i++) {
            $content .= '<TR class="' . (0 != $i % 2 ? 'bg2' : 'bg3') . '">
                <TD>' . unofficial_getDBResult($result, $i, 'skill_name') . '</TD>
                <TD>' . unofficial_getDBResult($result, $i, 'level_name') . '</TD>
                <TD>' . unofficial_getDBResult($result, $i, 'year_name') . '</TD></TR>';
        }
...
function people_show_category_table()
{
    ...
    $content = "<TABLE cellspacing='1' cellpadding='5' width='100%' border='0'>" . "<TR class='bg2'>" . '<TD><b>' . _XF_PEO_CATEGORY . '</b><p></td>' . '</TR>';

    $sql = 'SELECT pjc.category_id, pjc.name, COUNT(pj.category_id) AS total '
           . 'FROM '
           . $xoopsDB->prefix('xf_people_job_category')
           . ' pjc '
           . 'LEFT JOIN '
    ...
    if (!$result || $rows < 1) {
        ...
        for ($i = 0; $i < $rows; $i++) {
            $content .= '<TR class="'
                        ...
                        . '"><TD><A HREF="'
                        . XOOPS_URL
                        . '/modules/xfjobs/?category_id='
                        . unofficial_getDBResult($result, $i, 'category_id')
                        . '">'
                        . unofficial_getDBResult($result, $i, 'name')
                        . '</A> ('
                        . unofficial_getDBResult($result, $i, 'total')
                        . ')</TD></TR>';
        }
    }
...
function people_show_job_list($result, $forProj = false)
{
    ...
    if ($rows < 1) {
        $content .= '<tr><td colspan="4"><h4><br>' . _XF_PEO_NONEFOUND . '</h4>' . $xoopsDB->error() . '</td></tr>';
    } else {
        for ($i = 0; $i < $rows; $i++) {
            ...
            $content .= "<tr class='"
                        ...
                        . "&job_id=$job_id'>"
                        . unofficial_getDBResult($result, $i, 'title')
                        . '</a></td><td>'
                        . unofficial_getDBResult($result, $i, 'category_name')
                        . '</td><td>'
                        . date($sys_datefmt, unofficial_getDBResult($result, $i, 'date'))
                        . "</td><td align='center'>";

            if (!$forProj) {
                $content .= "<a href='" . XOOPS_URL . '/modules/xfmod/project/?' . unofficial_getDBResult($result, $i, 'unix_group_name') . "'>" . unofficial_getDBResult($result, $i, 'group_name') . '</a>';
            } else {
                $content .= "<a href='" . XOOPS_URL . '/modules/xfjobs/editjob.php?' . "group_id=$group_id&job_id=$job_id'>edit</a>/" . "<a href='" . XOOPS_URL . '/modules/xfjobs/viewjob.php?' . "group_id=$group_id&job_id=$job_id&deljob=1'>delete</a>";
            }
