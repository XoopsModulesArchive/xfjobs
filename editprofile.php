...
$GLOBALS['xoopsOption']['template_main'] = 'xfjobs_editprofile.html';
...
if ($update_profile) {
    ...
    */
    ...
    $sql = 'UPDATE ' . $xoopsDB->prefix('xf_user_profile') . " SET people_view_skills='$people_view_skills',resume='" . $ts->addSlashes($resume) . "' " . "WHERE user_id='" . $xoopsUser->getVar('uid') . "'";

    $result = $xoopsDB->queryF($sql);

    if (!$result) {
        $xoopsForgeErrorHandler->addError(
            'User update FAILED - ' . $xoopsDB->error()
        );
    } else {
    ...
    */
    ...
    if (100 == $skill_id || 100 == $skill_level_id || 100 == $skill_year_id) {
        //required info
    ...
    */
    ...
    if (100 == $skill_level_id || 100 == $skill_year_id || !$skill_inventory_id) {
        //required info
    ...
    $sql = 'UPDATE ' . $xoopsDB->prefix('xf_people_skill_inventory') . ' SET ' . "skill_level_id='$skill_level_id'," . "skill_year_id='$skill_year_id' " . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . "AND skill_inventory_id='$skill_inventory_id'";

    $result = $xoopsDB->queryF($sql);

    if (!$result) {
        $xoopsForgeErrorHandler->addError(
            'User Skill update FAILED - ' . $xoopsDB->error()
        );
    } else {
    ...
    */
    ...
    $sql = 'DELETE FROM ' . $xoopsDB->prefix('xf_people_skill_inventory') . ' ' . "WHERE user_id='" . $xoopsUser->getVar('uid') . "' " . "AND skill_inventory_id='$skill_inventory_id'";

    $result = $xoopsDB->queryF($sql);

    if (!$result) {
        $xoopsForgeErrorHandler->addError(
            'User Skill Delete FAILED ' . $xoopsDB->error()
        );
    } else {
...
<INPUT TYPE="RADIO" NAME="people_view_skills" VALUE="0" ' . ((0 == $people_view_skills) ? 'CHECKED' : '') . '> <B>' . _NO . '</B><BR>
<INPUT TYPE="RADIO" NAME="people_view_skills" VALUE="1" ' . ((1 == $people_view_skills) ? 'CHECKED' : '') . '> <B>' . _YES . '</B><BR>
<P>' . _XF_PEO_GIVEUSSOMEINFO . '
<P>
<B>' . _XF_PEO_RESUME . ':</B><BR>
<TEXTAREA NAME="resume" ROWS="15" COLS="60" WRAP="SOFT">' . $ts->htmlSpecialChars($resume) . '</TEXTAREA>
<P>
